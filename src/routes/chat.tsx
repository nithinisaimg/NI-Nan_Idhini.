...
import { analyzeEmotion, type AnalyzeResponse } from "@/lib/analyze.functions";
  ...
  head: () => ({
    meta: [
      { title: "Session · NI" },
      {
        name: "description",
        content:
          "Describe how you feel. NI's NLP and ML pipeline returns a calm response and solo self-care steps.",
      },
      ...
      <div className="mt-6 space-y-8 min-h-[40vh]">
        {turns.length === 0 && (
          <div className="border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
            Describe how you feel right now. Anything is fine — a sentence, a paragraph, a single
            word.
          </div>
        )}
        {turns.map((t, i) => (
    ...
    <div className="space-y-4">
      {r.risk_flag && r.emergency_notice && (
        <div className="border border-foreground bg-foreground text-background p-5">
          <div className="text-[10px] tracking-[0.3em] uppercase mb-2 opacity-70">
            Safety override
          </div>
          ...
          <div className="mt-4 grid sm:grid-cols-3 gap-2 text-[11px]">
            <a
              href="tel:112"
              className="border border-background/40 px-3 py-2 text-center hover:bg-background hover:text-foreground transition"
            >
              Emergency · 112
            </a>
            <a
      ...
      <div className="grid grid-cols-2 gap-px bg-border border border-border md:grid-cols-4">
        ...
        <Stat k="Top Cue" v={r.detected_keywords.length > 0 ? r.detected_keywords[0] : "None"} />
      </div>
        ...
        {r.detected_keywords.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {r.detected_keywords.map((k, i) => (
              <span
                key={i}
                className="text-[10px] tracking-wider uppercase border border-border px-2 py-1 text-muted-foreground"
              >
                {k}
              </span>
            ))}
      ...
      {r.analysis_summary && (
        <div className="border border-border p-4 bg-surface">
          ...
          <div className="text-sm leading-relaxed">{r.analysis_summary}</div>
        </div>
      )}
          ...
          <div className="divide-y divide-border">
            {r.solo_recommendations.map((rec, i) => (
              <div key={i} className="p-5 flex gap-4">
                <div className="text-foreground/40 tabular-nums text-xs w-6 mt-0.5">
  ...
  return (
    <div className="bg-background p-4">
      ...
      <div className="mt-2 text-base flex items-center gap-2">
        {accent && (
          <span
            className={`h-1.5 w-1.5 ${accent === "high" ? "bg-destructive" : accent === "medium" ? "bg-foreground" : "bg-muted-foreground"}`}
          />
        )}
        {v}
      </div>
